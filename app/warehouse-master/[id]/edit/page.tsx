...
type WarehouseMaster = Database['public']['Tables']['warehouse_master']['Row'];
...
export default function EditWarehousePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [whResult, wmsResult] = await Promise.all([
          supabase.from('warehouse_master').select('*').eq('id', id).single<WarehouseMaster>(),
          supabase.from('wms_master').select('id, wms_name').order('wms_name'),
        ]);
        if (whResult.error) throw whResult.error;
        if (whResult.data) {
          setFormData({
            warehouse_name: whResult.data.warehouse_name,
            wms_id: whResult.data.wms_id,
            remarks: whResult.data.remarks || '',
          });
        }
        if (wmsResult.data) setWmsList(wmsResult.data);
      } catch (err) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <h1 className="text-3xl font-bold text-gray-900">倉庫マスタ編集</h1>
      </div>
