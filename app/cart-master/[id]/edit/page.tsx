...
export default function EditCartPage() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <Link href="/cart-master" className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-700 mb-4">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
        <div>
          ...
          <input type="text" id="cart_name" required value={formData.cart_name}
            onChange={(e) => setFormData({ ...formData, cart_name: e.target.value })}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            placeholder="例: サイテキカート" />
          <p className="text-sm text-gray-500 mt-1">カート・ECサイトの名称を入力してください</p>
        </div>

        <div>
          <label className="flex items-center gap-3">
            <input type="checkbox" checked={formData.has_project_info_in_csv}
        ...
        <div className="flex items-center gap-4 pt-4 border-t border-gray-200">
          ...
          <Link href="/cart-master" className="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">キャンセル</Link>
        </div>
      </form>
    </div>
  );
